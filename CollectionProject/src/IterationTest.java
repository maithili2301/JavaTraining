...
class Singer implements Singing { //isA
	...
	public Reward  singASong(String hint) {
		...
		if(hint.equalsIgnoreCase("holidays")) {
			...
			reward = new Cash();
		}else if(hint.equalsIgnoreCase("cupsandballs")) {
			reward=new Cash();
		}
		return reward;
	}
...
class EventAgency {
	public static Magician getMagician(String hint) {
		...
		else if(hint.equalsIgnoreCase("cup")) {
			magician=new CupAndBallsMagician();
		}else if(hint.equalsIgnoreCase("balls")) {
			magician=new CupAndBallsMagician();
		}
		return magician;
	}
...
class CupAndBallsMagician implements Magician{

	@Override
	public Magic doTrick(Account accRef) {
		System.out.println("Doing Tricks with cups and balls.....");
		Magic magic=new CupAndBallsMagic();
		accRef.deposit(1500);
		return magic;
	}
...
class CupAndBallsMagic implements Magic{

	@Override
	public void show() {
		System.out.println("Showing Cups and balls magic.......");
